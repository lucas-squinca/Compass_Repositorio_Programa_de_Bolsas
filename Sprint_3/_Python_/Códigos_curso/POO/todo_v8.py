...
def main():
    ...
    #casa.procurar('Trocar lençóis').concluir() # .concluir não está funcionando
    print(casa)

    try: # Usado quando um código pode gerar um problema, tentamos rodá-lo, caso contrário (exceção):
        casa.procurar('Lavar prato - ERRO').concluir()
    except TarefaNaoEncontrada as erro:
        print(f'A causa foi "{str(erro)}"!')
